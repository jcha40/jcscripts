...
def logbetainc(a, b, x, n=20):
    ...
    if isinstance(a, (int, float)) and isinstance(b, (int, float)) and isinstance(x, (int, float)):
        if (a + 1.) * (1. - x) < (b + 1.) * x and x >= _min_x_swap:
    ...
    mask = ((a + 1.) * (1. - x) < (b + 1.) * x if x >= _min_x_swap else zeros_like(a + b + x, dtype=bool)) if isinstance(x, (int, float)) else ((a + 1.) * (1. - x) < (b + 1.) * x) & (x >= _min_x_swap)
